...
exports = (user) => {
  const collection = context.services.get("mongodb-atlas").db("PlantHealthcare").collection("users");
  const doc = collection.insertOne({ user_id: user.id, name: user.data.name, role: user.data.role });
};